	...
	if (token->Content.transfer.address.format == 0) {   //numeric constant expression
		...
		if (expression <= 0xff) {
			...
			*binary = 0;
			...
			uint16_t offset = (last_address + *ldr_count) * 4 - address - 8;            //PC is ahead with 8 bytes of instructions
...
void address_to_bits(Address address, uint32_t *binary) {
	...
	printf("\n\nRNNNNNNNNNNNNNNNN: %d\n",address.Expression.expression > 0);     // base register
	...
	if (address.Expression.Register.format == 0) {
		if (address.Expression.expression >= 0) {
			...
			printf("\n\nRNNNNNNNNNNNNNNNN: %d\n", (uint32_t)(-1 * address.Expression.expression));     // base register
